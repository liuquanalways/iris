...
func (db *Database) cleanup() error {
	return db.Service.Update(func(tx *bolt.Tx) error {
		...
		for bsid, v := c.First(); bsid != nil; bsid, v = c.Next() {
			...
			if bExp := b.Bucket(expirationName); bExp != nil {
	...
	err := db.Service.Update(func(tx *bolt.Tx) (err error) {
		...
		if b == nil {
			...
			if err := b.Put(expirationKey, timeBytes); err == nil {
				// just create the session bucket now, so the rest of the calls can be easly get the bucket without any further checks.
		...
		if expValue == nil {
			golog.Debugf("expiration value is nil for '%s'", sid) // this should never happen.
			return errNotFound
		}
...
func (db *Database) Release(sid string) {
	db.Service.Update(func(tx *bolt.Tx) error {
		// delete the associated the expiration bucket and the session bucket itself.
		b := db.getBucket(tx)
		bsid := []byte(sid)
		if err := b.DeleteBucket(getExpirationBucketName(bsid)); err != nil {
			return err
		}
		return b.DeleteBucket(bsid)
	})
